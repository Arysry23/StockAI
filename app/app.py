...
from utils.model_utils import generate_recommendation, calculate_accuracy

app = Flask(__name__)

# List of stock symbols
stock_symbols = [  'RELIANCE.NS', 'TCS.NS', 'INFY.NS', 'HINDUNILVR.NS', 'HDFCBANK.NS', 'ICICIBANK.NS', 'SBIN.NS', 'ITC.NS',  'BAJFINANCE.NS', 'KOTAKBANK.NS', 'ASIANPAINT.NS', 'MARUTI.NS', 'LT.NS', 'AXISBANK.NS', 
...
def daily_predictions():
    for symbol in stock_symbols:
        generate_recommendation(symbol)

# Schedule daily predictions at a specific time (e.g., 9:00 AM)
schedule.every().day.at("19:00").do(daily_predictions)

@app.route('/')
def index():
    predictions = {symbol: generate_recommendation(symbol) for symbol in stock_symbols}
    accuracy_scores = {symbol: calculate_accuracy(symbol) for symbol in stock_symbols}
